    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 rounded-lg bg-gray-800 shadow-inner max-h-96">
        {messages.map((message, index) => (
          ...
          >
            <div className={`max-w-[90%] ${message.type === 'user'
              ...
              <div className="flex items-start space-x-2">
                ...
                <div className="flex-1">
                  ...
                  {message.feedback && (
                    <div className="mt-2 p-2 bg-gray-600 rounded-lg text-xs text-gray-200">
                      <p className="font-semibold">Feedback:</p>
                      <p>{message.feedback}</p>
                      {message.improvement_tip && (
                        <p className="mt-1"><span className="font-semibold">Tip:</span> {message.improvement_tip}</p>
                      )}
                      {message.rating !== undefined && (
                        <p className="mt-1"><span className="font-semibold">Rating:</span> {message.rating}/5</p>
                      )}
                    </div>
                  )}
...
const InterviewRoom: React.FC = () => {
  ...
  const startInterviewWithGreeting = (session: SessionData) => {
    ...
    const greetingMessage = `Hello ${userName}! I'm ready to start your interview with ${session.questions.length} questions. Let's begin immediately.`;
  ...
  const submitAnswer = async (orderId: number, answerTextFallback: string, timeTaken: number, audioBlob: Blob | null) => {
    ...
    try {
      ...
      console.log("✅ Answer submitted successfully, received feedback:", data);
      return data;
  ...
  const getInterviewSummary = async () => {
    ...
    try {
      ...
      const fallbackMessage = "Interview completed. Thank you! Redirecting...";
  ...
  const endSession = async () => {
    console.log("🔚 Ending session (Early Exit)");

    // If we are currently reviewing, or even if mid-interview, 
    // we should try to generate a summary for what has been done so far 
    // OR just redirect to dashboard if that's what "Hang Up" implies.
    // User requested: "whenever i handup the interview then immediately stop and cancel the whole interview" -> "Finish current state"
    ...
    // Show summary of whatever was completed
    await completeInterview();
  };

  const endSessiondir = () => {
    console.log("🔚 Ending session (manual)");
    ...
    showSystemMessage("Interview session completed. Redirecting to dashboard...", 1000);
      ...
      >
        ...
        <div className="w-full h-full flex items-center justify-center relative">
          <div className="text-center">
            <motion.div
              className={`w-32 h-32 mx-auto mb-4 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center ${isSpeaking ? 'animate-pulse scale-110' : ''
                }`}
        ...
        {isInterviewActive && timeLeft > 0 && (
          <motion.div
            className={`absolute top-4 right-4 flex items-center px-4 py-2 rounded-full text-lg font-bold z-10 ${timeLeft <= 10 ? 'bg-red-600 animate-pulse' : timeLeft <= 30 ? 'bg-yellow-600' : 'bg-blue-600'
              } text-white`}
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          >
